...
function firstCross(length){a.reset();a.s.attached=true;a.s.length=length;Object.assign(a.s.load,{x:190+length*Math.sin(.2),y:80+length*Math.cos(.2),vx:0,vy:0});for(let i=0;i<600;i++){a.step();if(a.s.load.x<190)return i/120}throw Error('No crossing')}
const shortCross=firstCross(120),longCross=firstCross(360);test('long cable has a slower pendulum period',()=>assert(longCross>shortCross*1.5));console.log('Quarter swing seconds',shortCross,longCross);
